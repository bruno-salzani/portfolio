...
export const Projects: React.FC = () => {
  ...
  const getProjectDescription = (desc?: string | { en: string; pt: string }, category?: string) => {
    if (!desc) return getGenericDescription(category || 'Other');
    ...
    <div className="w-full">
      <div className="flex flex-wrap justify-center gap-3 mb-16">
        {(['All', 'QA', 'Other'] as const).map((tab) => (
          ...
          >
            {tab === 'All' ? t('projects.tab.all') : tab === 'QA' ? t('projects.tab.qa') : t('projects.tab.other')}
          </button>
        ))}
      </div>
          ...
          >
            ...
            <div className="relative z-10 h-full flex flex-col">
              ...
              <p className="text-xs text-slate-500 mb-6 line-clamp-3 leading-relaxed font-medium">
                {getProjectDescription(project.description, project.category)}
              </p>
