...
export const Experience: React.FC = () => {
  ...
  const getLiteral = (val: string | { en: string; pt: string }) => {
    if (typeof val === 'string') return val;
    return val[language];
  };

  const getDescription = (val: string[] | { en: string[]; pt: string[] }) => {
      ...
      {EXPERIENCES.map((exp, index) => (
        ...
        >
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
            <div>
              <h3 className="text-xl font-black text-white mb-1 tracking-tight uppercase">{getLiteral(exp.role)}</h3>
          ...
          <div className="text-[10px] font-bold text-slate-500 mb-6 uppercase tracking-widest flex items-center gap-2">
            ...
            {getLiteral(exp.location)}
          </div>

          <ul className="space-y-4">
            {getDescription(exp.description).map((item, i) => {
